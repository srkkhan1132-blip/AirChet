...
public class SignupActivity extends AppCompatActivity {
        ...
        etUsername.addTextChangedListener(new android.text.TextWatcher() {
            ...
            @Override public void onTextChanged(CharSequence s, int a, int b, int c) {
                ...
                db.collection("users").document("user_" + u).get().addOnSuccessListener(doc -> {
                    if (!doc.exists()) {
                        tvStatus.setText("Available: @" + u);
                        tvStatus.setTextColor(0xFF00897B);
                        usernameOk = true;
                    } else {
                        tvStatus.setText("Taken: @" + u);
    ...
    private void save() {
        ...
        db.collection("users").document(uid).set(user).addOnSuccessListener(v -> {
            prefs.edit().putString("uid", uid).putString("name", name).putString("username", username).apply();
